# StreamPlayerCore

[![NuGet](https://img.shields.io/badge/NuGet-StreamPlayerCore-004880?logo=nuget&logoColor=fff)](https://www.nuget.org/packages/StreamPlayerCore/)
[![NuGet](https://img.shields.io/badge/NuGet-StreamPlayerCore.WPF.Control-004880?logo=nuget&logoColor=fff)](https://www.nuget.org/packages/StreamPlayerCore.WPF.Control/)
[![NuGet](https://img.shields.io/badge/NuGet-StreamPlayerCore.WinForms.Control-004880?logo=nuget&logoColor=fff)](https://www.nuget.org/packages/StreamPlayerCore.WinForms.Control/)
[![NuGet](https://img.shields.io/badge/NuGet-StreamPlayerCore.Helper-004880?logo=nuget&logoColor=fff)](https://www.nuget.org/packages/StreamPlayerCore.Helper/)

## Components

### StreamPlayerCore

.NET library for video streaming using FFmpeg. Provides core functionality for handling video streams.

### StreamPlayerCore.WPF.Control

WPF control for video streaming using FFmpeg. Uses the core library to provide a user interface for video playback in WPF applications.

### StreamPlayerCore.WinForms.Control

WinForms control for video streaming using FFmpeg. Uses the core library to provide a user interface for video playback in WinForms applications.

## License

This project is licensed under the LGPLv3 License. See the [LICENSE](https://github.com/BlazekWasTaken/StreamPlayerCore/blob/2.1.4/LICENSE) file for details.