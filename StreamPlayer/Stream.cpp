...
{
    auto streamPtr = static_cast<Stream*>(ctx);
    ...
    RtspFlags flags, int32_t analyzeDuration, int32_t probeSize)
{
    ...
    else if (transport == UdpMulticast)
    {
...
void Stream::Open()
{
    static std::once_flag flag; // Specify std:: namespace
    std::call_once(flag, []()
    {
        // av_register_all(); // Removed: deprecated in recent FFmpeg
    ...
    for (uint32_t i = 0; i < formatContext->nb_streams; i++)
    {
        ...
        {
            videoStreamIndex_ = static_cast<int32_t>(i); // Explicit cast
    ...
    const AVCodec* codecPtr = avcodec_find_decoder(videoStreamPtr->codecpar->codec_id);
    if (codecPtr == nullptr)
    {
    ...
    videoTimer_ = static_cast<double>(av_gettime()) / 1000000.0; // Avoid narrowing warning
...
void Stream::Read()
{
    constexpr int32_t maxPacketQueueSize = 256;
    constexpr int32_t readerSleepTimeInMilliseconds = 10;
...
double Stream::GetTimestamp(const AVFrame* avframePtr, const AVRational& timeBase)
{
    double timestamp = 0.0;
    if (avframePtr->pkt_dts != AV_NOPTS_VALUE)
    {
        timestamp = static_cast<double>(avframePtr->best_effort_timestamp) * av_q2d(timeBase); // Explicit cast
    }
...
unique_ptr<Frame> Stream::CreateFrame(AVFrame* avframePtr)
{
    ...
    auto framePtr = boost::make_unique<Frame>(codecContext_->width,
...
int32_t Stream::GetInterframeDelayInMilliseconds(double frameTimestamp)
{
    ...
    double actual_delay = videoTimer_ - (static_cast<double>(av_gettime()) / 1000000.0);
    ...
    return static_cast<int32_t>(lround(actual_delay * 1000));
}
